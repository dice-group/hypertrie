...
namespace einsum::internal::util {
	...
	public:
		...
		[[nodiscard]] std::vector<WeakComponentLabels_t> getWeaklyConnectedComponentsLabels() {
			...
			boost::copy_graph(graph, undirected_graph);
			// stores by position (Vertex) which component it belongs to (entry value)
			std::vector<WeakComponentID> component(boost::num_vertices(undirected_graph));
			auto num_components = boost::connected_components(undirected_graph, &component[0]);
