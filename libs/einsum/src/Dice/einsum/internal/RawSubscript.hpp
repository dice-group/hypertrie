...
namespace Dice::einsum::internal {
	...
	public:
		 ...
		 */
		RawSubscript(const OperandsSc &operands, const ResultSc &result) : operands(operands), result(result),
																		   hash(Dice::hash::DiceHashxxh3<std::tuple<OperandsSc, ResultSc>>()(std::make_tuple(operands, result))) {}
		...
		RawSubscript(const PAIR_t<OperandsSc, ResultSc> &raw_subscript) : RawSubscript(std::get<0>(raw_subscript), std::get<1>(raw_subscript)) {}
		...
		[[nodiscard]] auto getOperandsLabelSet() const noexcept {
			tsl::sparse_set<Label> operand_labels{};
		...
		[[nodiscard]] auto getResultLabelSet() const noexcept {
			tsl::sparse_set<Label> result_labels{};
		...
		[[nodiscard]] auto getLabelPossInResult() const noexcept {
			tsl::sparse_map<Label, LabelPos> label_poss_in_result{};
...
struct std::hash<::Dice::einsum::internal::RawSubscript> {
	size_t operator()(const ::Dice::einsum::internal::RawSubscript &s) const { return s.hash; }
};
