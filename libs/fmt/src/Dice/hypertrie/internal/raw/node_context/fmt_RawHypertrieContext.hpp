...
namespace fmt {
	...
	struct formatter<::Dice::hypertrie::internal::raw::RawHypertrieContext<depth, tri_t>> : ::Dice::hypertrie::internal::util::SimpleParsing {
		template<typename FormatContext>
		auto format(::Dice::hypertrie::internal::raw::RawHypertrieContext<depth, tri_t> const &rnc, FormatContext &ctx) {
			...
			hana::for_each(hana::range_c<size_t, 1, depth + 1>, [&](auto depth_) {
				...
				if constexpr (not(height == 1 and ::Dice::hypertrie::internal::raw::HypertrieCoreTrait_bool_valued_and_taggable_key_part<tri_t>))
