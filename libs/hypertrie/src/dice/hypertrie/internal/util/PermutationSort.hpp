...
namespace dice::hypertrie::internal::util::sort_permutation {


	using SortPermutation = std::vector<std::size_t>;

	inline SortPermutation getDefault(size_t n) noexcept {
	...
	template<typename T>
	SortPermutation get(std::vector<T> const &vec) noexcept {
	...
	template<typename T>
	void apply(std::vector<T> &vec, SortPermutation const &p) noexcept {
