...
namespace Dice::einsum::internal {
	...
	template<hypertrie::HypertrieTrait_bool_valued tr>
	struct CardinalityEstimation {
		 ...
		 * @return
		 */
		static Label getMinCardLabel(const std::vector<::Dice::hypertrie::const_Hypertrie<tr>> &operands,
									 const std::shared_ptr<Subscript> &sc,
									 [[maybe_unused]] std::shared_ptr<Context> const &context) {
			const tsl::sparse_set<Label> &operandsLabelSet = sc->getOperandsLabelSet();
			const tsl::sparse_set<Label> &lonely_non_result_labels = sc->getLonelyNonResultLabelSet();
		...
		static double estimate(
				const std::vector<::Dice::hypertrie::const_Hypertrie<tr>> &operands,
				const std::shared_ptr<Subscript> &sc,
				[[maybe_unused]] std::shared_ptr<Context> const &context) {
			const tsl::sparse_set<Label> &operandsLabelSet = sc->getOperandsLabelSet();
			const tsl::sparse_set<Label> &lonely_non_result_labels = sc->getLonelyNonResultLabelSet();
	...
	protected:
		 ...
		 */
		static double calcCard(const std::vector<::Dice::hypertrie::const_Hypertrie<tr>> &operands, const Label label,
							   const std::shared_ptr<Subscript> &sc) {
