...
namespace dice::hypertrie::tests::utils {
	template<size_t depth,
			 HypertrieTrait htt_t,
			 typename htt_t::key_part_type max_key_part = typename htt_t::key_part_type(1),
			 typename htt_t::key_part_type min_key_part = typename htt_t::key_part_type(1)>
	class SingleEntryGenerator {
	public:
		using value_type = typename htt_t::value_type;
		using key_part_type = typename htt_t::key_part_type;

		using SinlgeEntry_t = ::dice::hypertrie::internal::raw::SingleEntry<depth, htt_t>;
		...
		SingleEntryGenerator &begin() noexcept {
			internal::raw::RawKey<depth, htt_t> key{};
	...
	public:
		...
		EntrySetGenerator &begin() noexcept {
			if constexpr (number_of_entries != 0) {
				...
				static internal::raw::RawKey<depth, htt_t> key{};
				key.fill(min_key_part);
	...
	template<size_t depth,
			 ...
			 HypertrieTrait htt_t,
			 typename htt_t::key_part_type max_key_part = typename htt_t::key_part_type(1)>
	class EntrySetGenerator_with_exclude : public EntrySetGenerator<depth, number_of_entries, htt_t, max_key_part> {
		using super_t = EntrySetGenerator<depth, number_of_entries, htt_t, max_key_part>;
