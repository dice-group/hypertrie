...
namespace hypertrie::tests::core::node {
	...
	TEST_CASE_TEMPLATE("Identifier", T,
		...
		SUBCASE(::hypertrie::internal::to_string<typename T::tr>().c_str()) {
			SUBCASE(fmt::format("depth = {}", depth).c_str()) {
				SUBCASE("default construct") {
					REQUIRE(Identifier_t().empty());
					REQUIRE(Identifier_t() == Identifier_t(std::initializer_list<Entry>{}));
				}

				SUBCASE("hash for single entry node") {
					RawKey_t key{};
					Identifier_t sen_id(Entry{key, {}});
				...
				SUBCASE("add and remove starting with compressed") {
					...
					Identifier_t id0(entries[0]);
					REQUIRE(id0 == Identifier_t(std::initializer_list<Entry>{entries[0]}));
					REQUIRE(id0.is_sen());
					SUBCASE("change value (1 entry)") {
						if constexpr (not tri::is_bool_valued) {
							...
							if (new_value == entries[0].value())
								new_value -= 1;
							REQUIRE(Identifier_t{id0}.changeValue(entries[0], std::numeric_limits<typename tri::value_type>::max()) == Identifier_t{Entry{entries[0].key(), new_value}});
						}
					}

					Identifier_t id1 = id0;
					id1.addEntry(entries[1]);
					REQUIRE(id1 == Identifier_t{entries[0], entries[1]});
					REQUIRE(id1.is_fn());
					if constexpr (not(depth == 1 and tri::taggable_key_part))
						REQUIRE(Identifier_t(id1).removeEntry(entries[1], true) == id0);
					SUBCASE("change value (2 entries)") {
						if constexpr (not tri::is_bool_valued) {
							auto new_value = std::numeric_limits<typename tri::value_type>::max();
							if (new_value == entries[1].value())
								new_value -= 1;
							REQUIRE(Identifier_t{id1}.changeValue(entries[1], std::numeric_limits<typename tri::value_type>::max()) == Identifier_t{entries[0], Entry{entries[1].key(), new_value}});
						}
					}

					Identifier_t id2 = id1;
					id2.addEntry(entries[2]);
					REQUIRE(id2 == Identifier_t{entries[0], entries[1], entries[2]});
					REQUIRE(id2.is_fn());
					if constexpr (not(depth == 1 and tri::taggable_key_part))
						REQUIRE(Identifier_t(id2).removeEntry(entries[2], false) == id1);
					REQUIRE(Identifier_t(id2).removeEntry(entries[2]) == id1);


					Identifier_t id3 = id2;
					id3.addEntry(entries[3]);
					REQUIRE(id3 == Identifier_t{entries[0], entries[1], entries[2], entries[3]});
					REQUIRE(id3.is_fn());
					if constexpr (not(depth == 1 and tri::taggable_key_part))
						REQUIRE(Identifier_t(id3).removeEntry(entries[3], false) == id2);
					REQUIRE(Identifier_t(id3).removeEntry(entries[3]) == id2);

					if constexpr (not(depth == 1 and tri::taggable_key_part))
						REQUIRE(Identifier_t(id3)
				...
				SUBCASE("use default sorting") {
					...
					Identifier_t id_empty{};
					Identifier_t id0(entries[0]);
					Identifier_t id1{entries[0], entries[1]};
					Identifier_t id2{entries[0], entries[1], entries[2]};
					Identifier_t id3{entries[0], entries[1], entries[2], entries[3]};
					std::vector<Identifier_t> ids{id_empty, id0, id1, id2, id3};
