...
namespace dice::hypertrie::internal::raw {
	...
	std::ostream &operator<<(std::ostream &os, RawKey<depth, htt_t> const &key) {
		os << fmt::format("{}", key);
		return os;
	}

	template<size_t depth, HypertrieTrait htt_t>
	std::ostream &operator<<(std::ostream &os, RawSliceKey<depth, htt_t> const &slice_key) {
		os << fmt::format("{}", slice_key);
		return os;
	}
} // namespace dice::hypertrie::internal::raw
