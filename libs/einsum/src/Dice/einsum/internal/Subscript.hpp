...
namespace Dice::einsum::internal {
	...
	private:
		mutable tsl::sparse_map<Label, std::shared_ptr<Subscript>> sub_subscripts{};

		RawSubscript raw_subscript{};

		tsl::sparse_set<Label> operands_label_set{};

		tsl::sparse_set<Label> result_label_set{};

		tsl::sparse_set<Label> lonely_non_result_labels{};
		...
		// Join
		mutable tsl::sparse_map<Label, LabelPossInOperands> label_poss_in_operands{};
		// Join & resolve
		mutable tsl::sparse_map<Label, LabelPos> label_poss_in_result{};
		// Resolve
		LabelPossInOperand operand2result_mapping_resolveType{};

		// Join
		tsl::sparse_map<Label, std::vector<OperandPos>> poss_of_operands_with_labels{};
	...
	public:
		...
		const tsl::sparse_set<Label> &getLonelyNonResultLabelSet() const {
			return lonely_non_result_labels;
		}

		const tsl::sparse_set<Label> &getOperandsLabelSet() const {
			return operands_label_set;
		}

		const tsl::sparse_set<Label> &getResultLabelSet() const {
	...
	public:
		...
		bool calcAllResultDone(const tsl::sparse_set<Label> &operand_labels,
							   const tsl::sparse_set<Label> &result_labels) {
	...
	private:
		Type calcState(const RawSubscript &raw_subscript,
					   const tsl::sparse_set<Label> &operands_label_set,
					   const tsl::sparse_set<Label> &result_label_set,
...
inline std::ostream &operator<<(std::ostream &stream, const std::shared_ptr<::Dice::einsum::internal::Subscript> &sub_script) {
	stream << sub_script->to_string();
	return stream;
}

inline std::ostream &operator<<(std::ostream &stream, std::shared_ptr<::Dice::einsum::internal::Subscript> sub_script) {
	stream << sub_script->to_string();
	return stream;
}

inline std::ostream &operator<<(std::ostream &stream, ::Dice::einsum::internal::Subscript const *const sub_script) {
	stream << sub_script->to_string();
	return stream;
}

inline std::ostream &operator<<(std::ostream &stream, const ::Dice::einsum::internal::Subscript &sub_script) {
	stream << sub_script.to_string();
	return stream;
}
...
//namespace fmt {
//	template<>
//	struct formatter<std::shared_ptr<::Dice::einsum::internal::Subscript>> {
//		template<typename ParseContext>
//		constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }
//
//		template<typename FormatContext>
//		auto format(const std::shared_ptr<::Dice::einsum::internal::Subscript> &sub_script, FormatContext &ctx) {
...
template<>
struct std::hash<::Dice::einsum::internal::Subscript> {
	size_t operator()(const ::Dice::einsum::internal::Subscript &s) const { return s.hash(); }
};
