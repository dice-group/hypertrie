...
#include <Dice/hypertrie/internal/node_based/interface/HashJoin.hpp>
...
namespace hypertrie::tests::hashjoin {
	...
	TEST_CASE("Compiles at least", "[HashJoin]"
		...
		Hypertrie<tr> tensor_0{1};
		tensor_0.set({0}, true);
		tensor_0.set({1}, true);
		tensor_0.set({2}, true);

		Hypertrie<tr> tensor_1{2};
		tensor_1.set({2, 7}, true);
		tensor_1.set({3, 8}, true);
		tensor_1.set({4, 9}, true);
		Hypertrie<tr> tensor_2{1};
		tensor_2.set({6}, true);
		tensor_2.set({2}, true);
		...
		std::vector<std::vector<pos_type>> op_sc{{1},
		                                         {1, 0},
		                                         {1}};
		std::vector<const_Hypertrie<tr>> operands = {tensor_0, tensor_1, tensor_2};
		HashJoin<tr> x{operands, {{0}, {0}, {0}}};
...
//
//			std::vector<HashJoin<tr>::poss_type> join_positions;
...
//				Hypertrie<tr> op{diagonal.depth};
...
//					HashJoin<tr> join{operands, join_positions};
//
