...
namespace einsum::internal {
    ...
    namespace {
        template<typename key_part_type, template<typename, typename> class map_type,
                template<typename> class set_type, typename const_BoolHypertrie, typename Diagonal>
        using Join = typename ::hypertrie::internal::interface::join<key_part_type, map_type, set_type>::template HashJoin<const_BoolHypertrie, Diagonal>;
    }
