...
namespace dice::einsum::internal::util {
	...
	public:
		 ...
		 */
		template<typename C>
		void addCompleteGraph(C const &new_nodes) noexcept {
			// add all combinations
			NodeSet new_nodes_set{new_nodes.begin(), new_nodes.end()};
			for (T const &node_a : new_nodes_set) {
				for (T const &node_b : new_nodes_set) {
		...
		[[nodiscard]] std::vector<NodeSet> getConnectedComponents() const noexcept {
			...
			while (not unfinished_nodes.empty()) {
				...
				while (not open_nodes.empty()) {
					...
					NodeSet const &adjacent_nodes = edges.at(node);
		 ...
		 */
		NodeSet neighbors(T const &node) const noexcept {
