...
namespace Dice::hash {
    ...
    std::size_t hash_and_combine(Ts &&...values) noexcept;
    ...
    template<typename... Ts>
    std::size_t hash_and_combine(Ts &&...values) noexcept {
        return xxh::xxhash3<detail::size_t_bits>(
                std::initializer_list<std::size_t>{dice_hash(std::forward<Ts>(values))...}, seed);
    }
